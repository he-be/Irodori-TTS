...
INPUTS: dict[str, dict[str, str | None]] = {
    ...
    # ~58 s of speech (latent ~1440): above the batch-3 ANE ceiling of 768 and the dev set's
    # largest bucket, so it exercises the batch-1 buckets 896-1536 (17-m1-ane-factors.md 5).
    "xlong": {
        "text": (
            "音声合成の速度を改善するためには、まず現状の処理時間を正確に測定し、"
            "どの段階に時間がかかっているのかを把握することが重要です。"
            "その上で、無駄な計算や同期を取り除き、GPUを効率的に使う工夫を一つずつ積み重ねていきます。"
            "最終的には、品質を保ったまま待ち時間を短くすることが目標です。"
            "測定では、短い文と長い文、参照音声の有無、キャプションの有無といった条件を分けて記録します。"
            "同じ条件で複数回繰り返し、中央値を取ることで、瞬間的な揺らぎに惑わされずに比較できます。"
            "改善の効果が小さいときほど、この地道な手順が結論の信頼性を支えてくれるのです。"
        ),
        "caption": None,
    },
}
