...
これは 13-ane.md 5-1 が M3 Pro で記録した「B=3 を 1536 frame まで列挙した package は、315 s かけて
コンパイルしたあと CPU 実行になり、**OS キャッシュにも乗らない（毎プロセス 315 s）**」と同じ署名である。
M3 Pro 側のログは未取得なので、エラー文言まで同じかは**未確認**。
...
5. **現行の `full` shape セットは M1 では使えない**（5-1 / 5-2）。6 package とも ANE コンパイルに失敗して
   ...
   失敗の要因（列挙数 / 最大形 / ctx / OS）は未分離で、「M1 には多 bucket が載らない」とはまだ言えない。
...
- **ANE コンパイル失敗の要因分離（最優先）**。5-2 の表のとおり、列挙数と最大形の 2 軸が混ざっている。
...
- 変換 93 分の内訳。`a_b2` 64 s に対し `a_b3` 1328 s と単調でなく、打ち切った `ANECompilerService` との
  CPU 競合やメモリ圧が混ざっている疑いがある（未確認）。
