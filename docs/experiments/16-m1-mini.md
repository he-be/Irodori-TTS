...
**訂正（09-22、17 節 4-2-1）: ANE 行の 1.97× は誤り。** 結果 JSON のタイムスタンプで確かめると、a の load + warmup（27 s）の
間に b の 5 回が走り終わり、a の 5 回は b の終了後に単独で走っていた（各プロセスの中央値が単独と同じなのはそのため。
p95 の 3568 / 3340 ms に重なった分だけ出ている）。2 プロセスが実際に重なった再測（09-22、再起動直後）では
**dev で 4790 / 4404 ms、スループット 1.14×**、MPS のみの行と同じ水準。ANE step も GPU decode も 2 倍近く伸びる。

MPS だけの構成では 2 本目がそのまま GPU の取り合いになり、各リクエストが 1.8 倍遅くなる（スループットはほぼ増えない）。
~~ANE 構成では片方の DiT（ANE）ともう片方の decode（GPU）が別の演算器で重なるため、各リクエストのレイテンシを
まったく落とさずにスループットがほぼ 2 倍になる。~~（上の訂正のとおり、同じ位相で投げた 2 プロセスは重ならない。）

~~15 節が「セグメント単位のヘテロ・パイプライン（導出 1.6×）」として構想していたものが、
2 ワーカーを並べるだけで実現している（実測 1.97×）。~~ → 実現していない（上の訂正）。パイプライン化するなら
15 節のとおり明示的に段を分ける必要がある。
...
3. **M1 では ANE の価値が M3 Pro より高い**。ステップ単体で 3 倍速い。
   ~~何より 2 ワーカーでスループットがほぼ 2 倍になる（MPS のみだと 1.12 倍）。~~ ← 誤り（09-22 訂正、4-7）。
   2 ワーカーのスループットは ANE でも 1.14 倍で MPS のみと変わらない。価値は単独レイテンシだけ。
