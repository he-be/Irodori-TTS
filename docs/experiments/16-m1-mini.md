...
作成: 2026-09-21 / 更新: 2026-09-22（`full` shape セットを一晩焼いた結果、5 節）（ブランチ `metal-local`、12〜15 の続き）
...
CPU で走る」現象が、M1 では **`full` セット全体で起きている**（compute plan は未取得なので、
CPU 落ちという**機序は未確認**。速度が CPU fp32 を下回る事実は実測）。
...
### 5-2. OS のコンパイルキャッシュは `full` サイズでは保持されない（実測）

ビルド完走後のベンチは、**プロセスを起動するたびに ANE コンパイルを払い直した**。

- 同じ `a_b1` を 5 回（昼のビルド、夜のビルド、その後のベンチ 3 本）焼いており、毎回 757〜766 秒。
- `a_b3` も 4 回、毎回 2017〜2088 秒。`a_b2` は 4474 → 4518 秒。
- 一方 `dev` セットは、ビルド後の別プロセスで **0.2〜1.2 秒**でロードできている（4-6）。

`dev`（3.8 GB）と `full`（8.2 GB）の差なので、**OS 側キャッシュの容量上限**を超えて
追い出されているという解釈が自然だが、**機序は未確認**（キャッシュの場所・上限は非公開）。
実用上の意味は明確で、**`full` は 1 リクエスト目までに 20〜75 分かかるので運用に載らない**。

**したがって 4-1 の ANE 行（`dev` セット）が M1 の到達点である。**
...
5. **`full` shape セットは使ってはいけない**（5-1 / 5-2）。M1 の ANE は 23 bucket の列挙を受け付けず
   CPU に落ち、ANE なしより 3 倍遅くなる。おまけに OS のコンパイルキャッシュも効かず、
   プロセス起動のたびに 20〜75 分の再コンパイルを払う。M3 Pro 向けの既定がそのまま害になる 3 つ目。
...
- **bucket 数の上限探索（最優先）**。`dev` の 3 bucket は載り、`full` の 23 bucket は載らない。
  その間のどこに閾値があるのかを、5 / 8 / 12 bucket のセットを足して `check_ane` で二分する。
  閾値が分かれば、パディングの無駄を最小にした M1 用のセットを本番既定にできる。
  併せて、閾値を決めているのが **bucket 数 / package サイズ / batch のどれか**（profile b を足せるか）。
- 5-2 の「キャッシュが保持されない」の機序と、`dev` サイズなら本当に恒久的に保持されるのか
