...
## 0. 引き継ぎ（2026-09-22 昼、set4 を mini に投入した直後）
...
`*_summary.txt` の末尾に `DONE` があればそのセットは完了。**現在走っているのは set4**（0-2）。
...
7. **次の実測（順に）**: (a) mini を再起動して `~/probe_logs/after_conc.sh` を再実行（dev / `m1` の 2 ワーカー、4-2）。
   (b) b_b3 × 6 形（profile b × batch 3）を 12 層で 1 本（長い本文 × CFG を ANE に載せる条件、4-3）。
   (c) `max_seconds` の既定 30 s のままなら b1 の bucket を 768 までに絞る（12 形。載るかは未測定、ビルド短縮の見込み）。
...
12 層 × b2（6 形）: 2×192 で 67 ms、2×768 で 387 ms。1 ブロックでは ANE / CPU = 4.9 / 11.8 ms（1×192）、51.5 / 96.8 ms（1×1536）。
...
2. **`m1` セット**（a_b1 / b_b1 × 18 形、a_b2 / a_b3 × 6 形）は M1 の ANE に載り（3 package 同時ロードを実測、a_b2 は
   単独で実測。ビルドは 4 package で約 53 分、導出）、e2e も長文も dev と**同速**（step 単体は 3〜5% 遅い）。
   `m1` が dev に対して買うのは守備範囲だが、その中身は当初の見込みより狭い: latent 768 超は既定の `max_seconds=30` では
   発生せず（4-3）、profile b は batch 1（CFG なし）にしか無い。**長い本文（64 トークン超）を CFG 付きで ANE に載せるには
   b_b3 が要る**（未測定）。現時点の実利は「2 候補生成（a_b2）」と「長いリファレンス × CFG なし（b_b1）」だけ。
3. **安全弁**（3 節）で、載らない package は初回の失敗コンパイル 1 回だけ払って以後 MPS に固定される。
   黙って CPU で 9 倍遅くなる（2-2 の 933 ms/step）ことは無くなった。
4. M1 での推奨: 速度だけなら `dev`（ビルド 35 分、16 節）で足りる。`m1`（4 package、約 53 分）は 2 候補生成や
   長いリファレンスを ANE に載せたいときだけ。`full` は指定しないこと（6 package × 12〜75 分の失敗コンパイルを初回に払う）。
5. **16 節 4-7 の「2 ワーカーでスループット 2 倍」は今日は dev でも再現しない**（4-2）。機械状態（swap 1.8 GB、壁紙の
   動画デコード常駐、1 日稼働）が疑わしく、再起動後の再測が要る。`m1` が悪いという結論は出せない。
