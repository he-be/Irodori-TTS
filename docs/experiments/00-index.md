...
| 12 | [12-igpu-offload.md](12-igpu-offload.md) | iGPU (Vega 7 / gfx90c) への TTS オフロード。sway 12 step 前提、CPU ベースライン RTF 1.7〜1.9、ROCm 6.3 wheel + gfx900 偽装、MIOpen の dilated conv 回避 (decode 21.8 → 1.6 s)、**iGPU fp16 で RTF 1.02〜1.12 / 3.7 GB**。carve-out は kernel 仕様で不可視 → TE=cpu + 上限 2560 で 2.95 GB に収め、`amdgpu.gttsize=4000` の再起動テスト待ち（**14 節に再開手順**） | 再起動待ち（続き 13） |
