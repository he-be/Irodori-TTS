...
| 13 | [13-igpu-dit-adaln-batch.md](13-igpu-dit-adaln-batch.md) | iGPU の DiT 高速化 (1): 全 24 AdaLN の低ランク射影を step ごとに bmm 2 回にまとめる（1 728 → 24 launch、メモリ増なし、LoRA/量子化時は自動 fallback）。iGPU で **short 6.80 → 6.55 s (RTF 1.017)、long 29.48 → 29.20 s**。dGPU fp32 は B=1 step の gemv/bmm 差 (fp32 eps) で hash が変わるため既定は ROCm のみ (`IRODORI_OPT_ADALN_BATCH=auto`)。Linear 融合の事前 microbench も収録 | 進行中（次: Linear 融合） |
