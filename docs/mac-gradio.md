...
# 長文を分割して順に読み上げ、1 本の wav にまとめる（6 節）
uv run python gradio_app_longform.py --server-port 7862
```

ブラウザで `http://127.0.0.1:7860` / `:7861` / `:7862`。停止は Ctrl-C（子プロセスの ANE worker も
一緒に落ちます）。

3 つとも起動時に以下を既定にしています（`gradio_app*.py` の冒頭、`os.environ.setdefault` なので
シェル側で指定すればそちらが勝ちます）:

```
IRODORI_OPT_ANE=1              ANE 経路を有効化
IRODORI_OPT_ANE_GPU_BRANCHES=1 cond 分岐を GPU に回す（Apple M1 では 0 が既定。GPU が律速になるため、16 節 4-4）
IRODORI_OPT_ANE_SHAPES=full    23 種の latent 長 × 2 プロファイル（Apple M1 では `m1` が既定、17 節 4-4）
...
## 6. 長文を読み上げる（gradio_app_longform.py）

原稿を 7〜12 秒のセグメントに割り、頭から順に生成して 1 本の wav にまとめる UI です。

```
原稿を貼る → [テキストを分割] → セグメント一覧（テキストは編集可）
          → [生成]（未生成 / 編集されたものだけ順に生成、終わったものから順に鳴らせる）
          → 読み間違いのセグメントだけテキストを直して [再生成]
```

「再生成」を押したセグメントと**結合ファイルだけ**が作り直されます。他のセグメントには触りません。
無音の長さを変えたときは「つなぎ直して書き出し」で、生成し直さずに貼り直せます。

### なぜ分割するのか

1 request の上限は 30 秒で、長いほど音声 1 秒あたりのコストが上がります（DiT は 7.2 / 11.8 / 28.8 s の
出力でそれぞれ 73 / 79 / 136 ms/s、20 秒を超えると step が自動で 16 に上がる）。**7〜12 秒が一番効率が
良い**（15-decode-ane.md 5-4）。加えて、読み間違いの直しが原稿全体ではなく 1 セグメントで済みます。

**実測**（4 セグメント / 音声 30 s の原稿、参照音声あり、既定設定、同一プロセス）:

| | wall | RTF |
|---|---:|---:|
| 1 巡目（先頭セグメントに compile 約 16 s が乗る） | 20.0 s | 0.67 |
| 2 巡目（同じ原稿を全再生成） | 5.8 s | 0.190 |
| セグメント単体（4.8〜11.6 s の音声） | 0.99〜2.00 s | 0.17〜0.22 |

3 分の原稿なら **35 秒前後**（導出、RTF 0.19）。最初のセグメントは 1〜2 秒で出来上がるので、
先頭から順に流す使い方にもそのまま乗ります。

8 節の参考値（short 1023 ms / RTF 0.142）より 15〜40% 遅いのは、セグメントごとに latent 長が変わり
ANE パッケージが切り替わるためだと思われますが、**未確認**です。15-decode-ane.md 5 節のパイプライン
並列（構成 C、1.6×）は**未実装**で、ここは素直な直列生成です。

### 分割の規則（`irodori_tts/text_segmentation.py`）

- **空行は段落の区切り**。必ずセグメント境界になり、そこだけ無音が 2 倍になります。
- 文末（`。!?！？…`）で切り、最大文字数まで詰めます。既定 60 文字 ≈ 12 秒
  （4.9 文字/秒、ベンチ入力での**実測**: 59 文字 → 11.8 s、140 文字 → 28.8 s）。
- 1 文が最大文字数を超えるときは読点（`、,;:`）で、それでも足りなければ機械的に切ります。
- 12 文字未満の断片は隣のセグメントに併合します（ただし 110 文字 ≈ 22 秒を超えない範囲）。
- 分割し直しても、**テキストが変わっていないセグメントの音声はそのまま残ります**。

### 注意点

- **声の一貫性は参照キャッシュ任せ**です。参照音声は一度上げたら触らないでください（5 節）。
  Seed は空欄ならセグメントごとにランダム、数値を入れるとセグメント i が `seed+i` で再現できます。
  「再生成」は毎回新しい seed を引くので、テキストを変えなくても別テイクになります。
- **継ぎ目の聴感（韻律・話速の揺れ）は未確認**です。セグメント間の無音（既定 0.25 秒）と前後の
  無音トリム（既定 on、そのセグメントのピーク −40 dB を基準に前後 50 ms 残す）で調整してください。
- 出力は `gradio_outputs_longform/run_YYYYmmdd_HHMMSS/` に、セグメントが `seg_XXX_vN.wav`、
  結合が `full_HHMMSS.wav`。結合ファイルは生成 / 再生成のたびに書き直されます。
- セグメント数の上限は 64（約 12 分）。それを超えるときは原稿を分けてください。
...
## 8. 参考値（実測、fp16、3 回中央値、13-ane.md 5 節 / 14-step-count.md 3 節）
